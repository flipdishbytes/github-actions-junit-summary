...
function collectSuites(doc: Record<string, unknown>): Record<string, unknown>[] {
...
function extractDetail(node: Record<string, unknown>): { message?: string; body?: string } {
 ...
 */
export function extractLocation(body: string | undefined): { file: string; line: number } | undefined {
...
export async function run(opts: RunOptions): Promise<RunResult> {
  ...
  if (files.length === 0) {
    ...
    const empty: RunResult = { total: 0, passed: 0, failed: 0, skipped: 0, files: 0, markdown: '' };
    setOutput('total', 0);
    setOutput('passed', 0);
    setOutput('failed', 0);
    setOutput('skipped', 0);
  ...
  for (const file of files) {
    ...
    try {
      ...
      log(`::error title=JUnit parse error::Failed to parse ${file}: ${(err as Error).message}`);
    }
  }
  ...
  for (const p of patterns) {
    ...
    for (const part of parts) {
      ...
      if (fs.existsSync(abs) && fs.statSync(abs).isDirectory()) {
        const found = await fg('**/*.xml', { cwd: abs, onlyFiles: true, absolute: false });
...
function emitAnnotation(log: (l: string) => void, c: TestCase): void {
  ...
  const msg = firstLine.substring(0, 500) || `${c.status === 'errored' ? 'Error' : 'Failure'} in ${c.name}`;
  const fields: string[] = [];
...
export async function runAsAction(): Promise<void> {
  ...
  const patterns = rawPath.split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
  ...
  try {
    ...
    core.setFailed(`junit-summary crashed: ${(err as Error).stack ?? (err as Error).message}`);
  }
}
